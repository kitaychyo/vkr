      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {matches.map((match) => (
            <tr 
              key={match.match_id} 
              className="border-b border-gray-800 hover:bg-card-hover transition-colors cursor-pointer"
              onClick={() => handleRowClick(match.match_id)}
            >
              <td className="py-3 px-4">
                <span className="text-accent font-mono text-xs underline">#{match.match_id}</span>
              </td>
              <td className="py-3 px-4">
